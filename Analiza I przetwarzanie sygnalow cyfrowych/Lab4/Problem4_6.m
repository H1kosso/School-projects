...
x = x1;                                 % wybor: x1, x2, x3, x1+x2, x2+x3
...
figure;
subplot(321); plot(fk,real(X),'o-'); title('real(X(f))'); grid;
subplot(322); plot(fk,imag(X),'o-'); title('imag(X(f))'); grid;

subplot(323); plot(fk,20*log10(abs(X)),'o-'); title('abs(X(f)) [dB]'); grid;
subplot(324); plot(fk,angle(X),'o-'); title('angle(X(f)) [rad]'); grid;
...
error2 = max(abs(x-y)),                 % blad odtworzenia sygnalu
                              % wynik
subplot(325); plot(real(y),'bo-'); title('real(y(n))'); grid;
subplot(326); plot(imag(y),'bo-'); title('imag(y(n))'); grid;
...
% Funkcja "okna"
w2 = chebwin(N,100);                          % okno prostokatne, N - dlugosc
w = w2; scale = 1/sum(w);               % wybor
...
% Funkcja "okna"
w2 = boxcar(N);                         % okno prostokatne, N - dlugosc
w = w2; scale = 1/sum(w);               % wybor
...
figure('Name',"w1 x1+x3 X(1+10)=0; X(N-10+1)=0;");
subplot(321); plot(fk,real(X),'o-'); title('real(X(f))'); grid;
subplot(322); plot(fk,imag(X),'o-'); title('imag(X(f))'); grid;
