...
if(0)  % ### dobor wartosci wspolczynnikow wielomianow transmitancji
   b = [0.01, 0.001];                         % [ b0, b1 ]
   a = [-0.61, 0.98];            % [ a0=1, a2, a3, a4]
   z = roots(b); p = roots(a);        % [b,a] --> [z,p]
else   % ### dobor miejsc zerowych wielomianow transmitancji
   gain = 0.001;
   z = [ 1 ] .* exp(j*2*pi*[ 500 ]/fpr);   z = [z conj(z)];  
   p = [ 0.9 ] .* exp(j*2*pi*[ 20 ]/fpr);   p = [p conj(p)]; 
        %zmienić na 1.1
...
x=zeros(1,Nx); x(1)=1;     % suma dwoch sinusow
...
figure;
subplot(211); plot(t,x); grid; title("x(n)");         % sygnal wejsciowy x(n)
subplot(212); plot(t,y); grid; title("y(n)");         % sygnal wyjsciowy y(n)
figure;                                 % widma FFT drugicj polowek sygnalow x(n) i y(n) (bez stanow przejsciowych)
k=Nx/2+1:Nx; f0 = fpr/(Nx/2); f=f0*(0:Nx/2-1);
subplot(211); plot(f,20*log10(abs(2*fft(x(k)))/(Nx/2))); grid; title("fft x");
subplot(212); plot(f,20*log10(abs(2*fft(y(k)))/(Nx/2))); grid; title("fft x");
